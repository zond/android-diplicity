...
APK_VERSION_LINE=`/usr/local/android-sdk-linux/build-tools/25.0.1/aapt dump badging ${APK_PATH}`
...
go run circle/create_release.go -apk=${APK_PATH} -build=${CIRCLE_BUILD} -tag=${APK_VERSION_CODE} -short_sha=${APK_VERSION_NAME}
